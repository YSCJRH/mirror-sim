...
- GitHub source-of-truth baseline:
  ...
  - `gh issue list --milestone "Phase 61 - Selected-World Review Surface Evidence Binding Gate" --state all`
    - `#471` `Phase 61 exit gate` is open with `lane:protected-core` and `status:blocked`
    - `#472` `Phase 61: sync repo truth after Phase 60 closeout and define review surface evidence gate` is open with `lane:protected-core` and `status:ready`
    - `#473` `Phase 61: add selected-world review surface evidence binding smoke` is open with `lane:protected-core` and `status:ready`
  - `python -m backend.app.cli audit-github-queue --repo YSCJRH/mirror-sim`
    - `audit-github-queue` reports `ready` with active milestone `Phase 61 - Selected-World Review Surface Evidence Binding Gate`
