...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Smoke-test Phase 59 selected-world route continuity with GET-only checks."
    )
    ...
    stdout_log = log_dir / f"phase59-selected-world-routes-smoke-{timestamp}.log"
    stderr_log = log_dir / f"phase59-selected-world-routes-smoke-{timestamp}.err.log"
    ...
    route_expectations = {
        ...
        # Empty session query keeps selected-world pages in deterministic no-session branches.
        "/worlds/fog-harbor-east-gate?session=": {
            "required": ["Fog Harbor East Gate", "Open perturb"],
            "forbidden": ["Launch Hub now", "Private Beta Launch Hub"],
        },
        "/worlds/fog-harbor-east-gate/review?session=": {
            "required": [
        ...
        "/worlds/museum-night?session=": {
            "required": ["Museum Night", "Open perturb"],
            "forbidden": ["Launch Hub now", "Private Beta Launch Hub"],
        },
        "/worlds/museum-night/review?session=": {
            "required": [
                "Museum Night",
                "Mirror Engine / Private Beta",
                "Generate one live branch first",
        ...
        "/worlds/library-rain?session=": {
            "required": ["Library Rain", "Open perturb"],
            "forbidden": ["Launch Hub now", "Private Beta Launch Hub"],
        },
        "/worlds/library-rain/review?session=": {
            "required": [
                "Library Rain",
                "Mirror Engine / Private Beta",
                "Generate one live branch first",
    ...
    try:
        ...
        result = {
            ...
            "mode": "phase59_selected_world_routes_get_only",
            "page_statuses": page_statuses,
            "route_count": len(page_statuses),
            "world_ids": ["fog-harbor-east-gate", "museum-night", "library-rain"],
            "boundary": (
                "narrow selected-world route continuity evidence only; no runtime mutation flow exercised"
            ),
        }
        if not remote_mode:
