...
- Phase 65 is active as `Phase 65 - Selected-World Deterministic Runtime Generation Evidence Gate`. Phase 64 is closed after PR `#494`, and `#489` `Phase 64 exit gate` closed by PR `#494`. The active queue contains `#495` `Phase 65 exit gate`, `#496` `Phase 65: sync repo truth after Phase 64 closeout and define selected-world runtime-generation evidence gate` (`#496` is handled by this Phase 65 sync PR), and `#497` `Phase 65: add selected-world deterministic runtime generation smoke`; `audit-github-queue` reports `ready` for the active Phase 65 milestone. The gate lives in `docs/plans/phase-65-selected-world-deterministic-runtime-generation-gate-2026-06-01.md` and covers selected-world deterministic runtime generation evidence through existing v1 CLI/session contracts with temporary local artifacts only.
