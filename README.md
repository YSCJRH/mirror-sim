...
- Phase 64 is active as `Phase 64 - Selected-World Perturb Follow-Up Readiness Gate`; `#489` `Phase 64 exit gate`, `#490` `Phase 64: sync repo truth after Phase 63 closeout and define selected-world perturb follow-up gate`, and `#491` `Phase 64: add selected-world perturb follow-up readiness smoke` define the active queue. Phase 63 is closed after PR `#488`. Phase 63 selected-world review next-action route fidelity is the historical baseline. The Phase 64 gate lives in `docs/plans/phase-64-selected-world-perturb-followup-readiness-gate-2026-05-26.md` and covers selected-world perturb follow-up readiness only: selected-world perturb routes must stay reachable, world-scoped, bound to world-local perturbation presets, and backed by decision schema defaults without starting sessions, generating branches, or calling POST/runtime APIs. The Phase 64 perturb follow-up readiness evidence lives in `docs/plans/phase-64-selected-world-perturb-followup-readiness-2026-05-26.md` and is reproduced by `scripts/smoke_phase64_selected_world_perturb_followup_readiness.py`.
